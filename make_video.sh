...
# -------------------------
# 2) VIDEO SHORT
# - Mantiene panel intacto al centro (sin crop del contenido)
# - Fondo 9:16 con blur de la misma imagen
# - MISMA DURACIÓN (sin truncar)
# -------------------------
ffmpeg -y -loop 1 -framerate $FPS -i "$IMG" \
  -t "$DUR" \
  -filter_complex "\
[0:v]zoompan=z='${ZOOM_EXPR}':d=1:s=1920x1080,fade=t=in:st=0:d=0.4,fade=t=out:st=${FADEOUT_START}:d=0.4,split=2[vmain][vbg]; \
...
# Audio para short (mismo audio, sin truncar)
if [ -f "$BGM" ]; then
